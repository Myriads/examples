/*
  Serial Call and Response (시리얼 요청과 응답)
 언어: Wiring/Arduino

 이 스케치는 ASCII 'A'(값이 65인 바이트) 문자를 시작할 때
 보내고, 컴퓨터가 데이터를 보낼 때까지 반복하여 보냅니다.
 시리얼 포트에 바이트 데이터가 들어올 때마다  3개의 센서 
 값들을 읽어 컴퓨터로 보냅니다.
...
void setup()
{
  ...
  pinMode(2, INPUT);   // 디지털 센서(스위치)가 디지털 2번 핀에
                       ...
                       // 설정합니다
  establishContact();  // 컴퓨터가 응답할 때까지 접속을 위한 바이트
                       // 데이터를 보냅니다
}
...
void loop()
{
  ...
  if (Serial.available() > 0) {
    // 보낸 바이트 데이터를 읽어 들입니다:
    inByte = Serial.read();
    // 첫번째 아날로그 센서를 읽고, 0~255 사이의 값으로 만들기 위하여
    // 4으로 나눕니다:
    firstSensor = analogRead(A0)/4;
    // ADC가 안정되게 10ms 동안 기다립니다:
    delay(10);
    // 두번째 아날로그 센서를 읽고, 0~255 사이의 값으로 만들기 위하여
    // 4으로 나눕니다:
