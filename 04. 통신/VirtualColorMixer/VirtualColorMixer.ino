/*
  Virtual Color Mixer (가상 색 혼합기)
  이 예제는 3개의 아날로그 센서들(가변 저항이 제일 쉬울 것입니다)을
 읽어 들이고, 그 값들을 시리얼로 전송합니다. 밑에 있는 Processing과
 Max/MSP 프로그램들은 보낸 세 값들을 받아들이고 화면의 배경색을 이
 값들로 변경합니다.
 
 회로:
 * 가변 저항들을 아날로그 0, 1, 그리고 2번 핀에 연결합니다
 
 http://www.arduino.cc/en/Tutorial/VirtualColorMixer
 
 2006년 12월 02일 David A. Mellis에 의하여 창작되었으며
 2011년 08월 30일 Tom Igoe와 Scott Fitzgerald에 의하여 수정되었습니다
 
  이 예제 코드는 공유 저작물입니다.
 */

const int redPin = A0;		// 빨간색을 조절하기 위한 센서
const int greenPin = A1;	// 초록색을 조절하기 위한 센서
const int bluePin = A2;		// 파란색을 조절하기 위한 센서
...
void loop()
{
  // 세 센서 값들을 읽어 들여 쉼표로 분리된 형태의 문자열로
  // 끝에는 새줄 기호를 붙여 보냅니다:
